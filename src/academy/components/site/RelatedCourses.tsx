...
export function RelatedCourses({ courses = [] }: RelatedCoursesProps) {
  ...
  return (
    <>
      <h2 className={cn(typography.sectionTitle, "mt-12 mb-5")}>{t("関連コース")}</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {courses.map((course) => {
          ...
          return (
            <a
              key={course.id}
              href={path(coursePath(course.title, course.id, course.slug))}
            ...
            >
              ...
              <div className="min-w-0">
                ...
                >
                  {t("コースを見る")} <ArrowRight className="w-3 h-3" />
                </span>
              </div>
            </a>
          );
